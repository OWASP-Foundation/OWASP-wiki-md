...
If the server responds with a 200 OK response for the first and a 404
Not Found for the second then it indicates that the server is running
the PL/SQL Gateway.
...
DBMS_\*, any request with HTP.\* or OWA\*. It is possible to bypass the
exclusion list however. What's more, the exclusion list does not prevent
access to packages in the CTXSYS and MDSYS schemas or others, so it is
possible to exploit flaws in these packages:
...
this sorry story can be found here:
...
DBMS_EXPORT_EXTENSION (see <http://secunia.com/advisories/19860>)
